...
Ocean gliders and acoustic doppler current profilers (ADCPs) are widely used tools for observing ocean circulation: gliders provide long-duration, autonomous sampling through the water column, while ADCPs measure water velocity over short ranges. When an ADCP is mounted on a glider, these complementary measurements can be combined to estimate vertical shear and reconstruct current profiles across the glider’s full operating depth. This processing is technically challenging because ADCP velocities are measured relative to a moving vehicle and across a short range (typically 15m, compared to glider dives of 1000m), the measurements from the glider and ADCP must be precisely aligned in time, space, and reference frame, and ADCP integration differs across glider platforms and beam configurations.

The gliderad2cp toolbox provides a cross-platform Python toolchain for processing Nortek AD2CP data collected from ocean gliders. It simplifies the workflow from raw ADCP and glider data to quality-controlled shear and referenced velocity profiles, supporting different glider implementations such as alternating three-beam configurations and four-beam configurations. The package handles file loading, quality control, vertical regridding to reduce shear smearing, coordinate transformations from beam-relative to Earth-relative velocities, vertical shear estimation, reconstruction of full-depth shear profiles, referencing to dive-averaged currents derived using the ADCP as a velocity logger, and shear-bias correction. By providing consistent processing across multiple glider platforms, gliderad2cp fills a gap left by existing ADCP or platform-specific toolboxes and makes glider-mounted ADCP observations easier to use for oceanographic analysis.
...
6. Reconstruct full-depth profiles of vertical shear from the successive short-range measurements of velocity. Simple gridding and integrating of shear is used as default rather than the inverse method due to the use of a single constraint [@visbeck:2002].

7. Reference the full high-resolution vertical shear profile using the glider's dive-averaged current to provide a high-resolution absolute measurements of ocean currents. The referencing of baroclinic profiles accounts for time spent at each depth in order to be compatible with repeat dives without surfacing or long glider loiters.

8. Optionally, perform a shear-bias correction, adapted from [@todd:2017], which aims to minimise the relation between net shear per profile and displacement through water. Previously published implementations [@todd:2017] minimised variance at depth which introduces biases in highly dynamic regions and fails to converge when gliders travel in a few different directions (*eg*. single long transects).
